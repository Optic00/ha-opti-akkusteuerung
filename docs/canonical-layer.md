...
Fehlt `opti_price_current_ct_kwh` **oder** die Preisreihe (< 4 verwertbare Werte, auch nach dem 15-Minuten-Haltefenster), wird `sensor.opti_price_level` über seinen Availability-Check `unavailable` (siehe [Abgeleitete Sensoren](#abgeleitete-sensoren-opti_derivedyaml)).
...
| `sensor.opti_price_series_stable` | Preisreihen-Halter (trigger-basiert): spiegelt `today`/`tomorrow` aus `opti_price_series` und überbrückt einen Quellausfall, indem er die letzte gültige Reihe **bis zu 15 Minuten** hält; danach fail-closed. State = `frisch` / `gehalten` / `leer`; `gehalten` deckt auch den Teilausfall ab, bei dem nur die Morgen-Liste wegfällt (Attribut `gehalten_teil` = `reihe` / `morgen` / `nichts`). Das Gedächtnis liegt getrennt von der Nutzlast auf `anker_today` / `anker_tomorrow` / `anker_ts` (nur intern, nicht von Konsumenten lesen). Einzige Preisreihen-Quelle für Preisniveau und Peak-Reserve |
...
| **Preisreihe fehlt ganz** | < 4 verwertbare Werte und kein Halter-Cache im Fenster (auch: Ausfall über 15 Minuten) | Preisniveau wird **`unavailable`**, `sensor.opti_peak_reserve_soc` ebenfalls | Fail-closed: alle preisabhängigen Zweige (Preis-/Winterladeblöcke, Peak-Leiter L1-L4) sind inaktiv. Kein `NORMAL`-Fallback mehr - der täuschte der Strategie bei einem Datenausfall ein gültiges Mittelpreis-Signal vor |
