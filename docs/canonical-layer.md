...
Fehlt `opti_price_current_ct_kwh` **oder** die Preisreihe (< 4 verwertbare Werte, auch nach dem Haltefenster), wird `sensor.opti_price_level` über seinen Availability-Check `unavailable` (siehe [Abgeleitete Sensoren](#abgeleitete-sensoren-opti_derivedyaml)).
...
| `sensor.opti_price_series_stable` | Preisreihen-Halter (trigger-basiert): spiegelt `today`/`tomorrow` aus `opti_price_series` und überbrückt einen Quellausfall, indem er die letzte gültige Reihe **bis zu 15 Minuten** hält (minütlich abgetastet); zusätzlich nur innerhalb des heutigen Kalendertags, weil ein 15-Minuten-Fenster Mitternacht überschreiten kann. Danach fail-closed. State = `frisch` / `gehalten` / `leer`; `gehalten` deckt auch den Teilausfall ab, bei dem nur die Morgen-Liste wegfällt (Attribut `gehalten_teil` = `reihe` / `morgen` / `nichts`). Das Gedächtnis liegt getrennt von der Nutzlast im Attribut `anker` (nur intern, nicht von Konsumenten lesen). Einzige Preisreihen-Quelle für Preisniveau und Peak-Reserve |
...
| **Preisreihe fällt kurz aus** | Quelle liefert `today`/`tomorrow` leer (z. B. Provider-Timeout) | `sensor.opti_price_series_stable` **hält** die letzte gültige Reihe bis zu 15 Minuten (State `gehalten`); Preisniveau und Peak-Leiter arbeiten unverändert weiter | Zulässig, weil die Tagesreihe ein Fahrplan ist und sich innerhalb des Tages nicht ändert. Das Fenster ist bewusst knapp - die beobachteten Ausfälle dauerten 20-80 s - siehe [strategie-logik.md](strategie-logik.md#fail-safes-und-bekannte-grenzen) |
| **Preisreihe fehlt ganz** | < 4 verwertbare Werte und kein Halter-Anker im Fenster (Ausfall über 15 Minuten oder Anker von gestern) | Preisniveau wird **`unavailable`**, `sensor.opti_peak_reserve_soc` ebenfalls | Fail-closed: alle preisabhängigen Zweige (Preis-/Winterladeblöcke, Peak-Leiter L1-L4) sind inaktiv. Kein `NORMAL`-Fallback mehr - der täuschte der Strategie bei einem Datenausfall ein gültiges Mittelpreis-Signal vor |
