...
| `sensor.opti_price_series_stable` | Preisreihen-Halter (trigger-basiert): spiegelt `today`/`tomorrow` aus `opti_price_series` und hält bei einem Quellausfall die letzte gültige Reihe **desselben Kalendertags**. State = `frisch` / `gehalten` / `leer`; `gehalten` deckt auch den Teilausfall ab, bei dem nur die Morgen-Liste wegfällt (Attribut `gehalten_teil` = `reihe` / `morgen` / `nichts`). Einzige Preisreihen-Quelle für Preisniveau und Peak-Reserve |
