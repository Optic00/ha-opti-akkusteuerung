...
|---|---|
| **P10-Sicherheitsnetz** | `sensor.opti_forecast_score`, `_tomorrow` und `sensor.opti_target_soc` verwenden das 10. Perzentil der Solcast-Prognose (`estimate10`) als konservativen Referenzwert — schützt vor Überoptimismus bei unsicheren Prognosen. Für heute (`opti_forecast_score` + `opti_target_soc`, über den zentralen Sensor `opti_forecast_effective_remaining_kwh`) ist der Grad per `input_number.opti_forecast_optimismus` einstellbar: 0 = `min(median, P10)` wie bisher, 100 = reiner Median. `opti_forecast_score_tomorrow` (Ganztags-Quelle) bleibt fest bei `min(median, P10)`. |
