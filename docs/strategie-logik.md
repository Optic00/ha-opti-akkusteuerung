...
- **Kurzer Ausfall der Preisquelle:** `sensor.opti_price_series_stable` überbrückt ihn und hält die letzte gültige Tagesreihe (State `gehalten`), Preisniveau und Leiter arbeiten unverändert weiter.
  ...
  Direkt nach Mitternacht kommt eine **Rollover-Wache** dazu: solange die Quelle noch exakt die gestrige Reihe zeigt, gilt nichts als frisch, damit die gestrigen Preise nicht auf den neuen Tag umdatiert werden.
  Sie greift nur in den Stunden 00-01, weil eine wertgleiche Folgetagsreihe (etwa wiederholter Flattarif) von der gestrigen nicht unterscheidbar ist und den Halter sonst den ganzen Tag blockieren könnte.
  Bekannte Grenze: eine Quelle, die über Stunden eine veraltete statt einer leeren Liste liefert, ist ohne Zeitstempel nicht erkennbar.
