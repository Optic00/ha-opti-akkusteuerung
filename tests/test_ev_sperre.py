...
def _lp1_sources():
    """Quell-Entity-IDs aus dem PRIVATEN Mapping extrahieren (Repo ist
    oeffentlich - echte IDs duerfen nicht in getrackten Tests stehen)."""
    cfg = load_yaml(MAPPING_PATH)
    entity = find_template_entity(cfg, "binary_sensor", "opti_mapping_ev_lp1_schnell")
    ids = re.findall(r"(?:select|binary_sensor)\.[a-z0-9_]+", entity["state"])
    mode = next(i for i in ids if i.startswith("select."))
    charging = next(i for i in ids if i.startswith("binary_sensor."))
    return entity, mode, charging


def _lp1_state(mode, charging):
    entity, mode_id, charging_id = _lp1_sources()
    hass = FakeHass(states={mode_id: mode, charging_id: charging})
...
def test_mapping_lp1_modusbasiert(mode, charging, erwartet):
    state, avail = _lp1_state(mode, charging)
    assert avail == "True"
...
def test_mapping_lp1_invalide_wird_unavailable():
    ...
    state, avail = _lp1_state("unavailable", "on")
    assert avail == "False"
